<script>
	...
	let price = $derived(
		game?.price_overview?.final_formatted ?? (game?.is_free ? "Free" : null),
	);
	...
	let categories = $derived(
		game?.categories?.slice(0, 6)?.map((c) => c.description) ?? [],
	);
	...
	let friendsInGame = $derived(
		friends.filter((f) => f.gameid && String(f.gameid) === String(appid)),
	);
	let criticScore = $derived(
		game?.metacritic?.score ?? game?.metacritic_score ?? null,
	);
	...
	let controllerSupportLabel = $derived(() => {
		const support = String(game?.controller_support ?? "")
		.trim()
		.toLowerCase();
	...
	let gameDetailRows = $derived(() => {
		const rows = [];
		if (game?.type) 
			rows.push({ label: "Type", value: titleCase(game.type) });
		if (releaseLabel())
			rows.push({
		...
		if (platformNames.length)
			rows.push({ label: "Platforms", value: platformNames.join(", ") });
		if (totalAch > 0)
			rows.push({ label: "Achievements", value: `${totalAch} total` });
		if (reviewTotal)
			rows.push({
				label: "Reviews",
				value: `${reviewTotal.toLocaleString()} reviews`,
			});
		if (game?.dlc_count > 0)
			rows.push({ label: "DLC", value: `${game.dlc_count} items` });
		...
		if (movies.length > 0)
			rows.push({ label: "Trailers", value: `${movies.length} available` });
		return rows;
	});
	let supportRows = $derived(() => {
		const rows = [];
		if (controllerSupportLabel())
		rows.push({ label: "Controller", value: controllerSupportLabel() });
		if (ageRating()) rows.push({ label: "Age Gate", value: ageRating() });
		if (supportedLanguages.length)
		rows.push({
			label: "Languages",
			value:
			supportedLanguages.slice(0, 2).join(", ") +
	...
	let modalSrc = $derived(
		modalIdx !== null ? (screenshots[modalIdx]?.path_full ?? null) : null,
	);
	...
	$effect(() => {
		...
		const state = get(db);
		const currentCachedGame =
		state?.cache?.library?.details?.[id]?.data ?? null;
		...
		const cachedAchievements = readViewCache(
		id,
		"achievements",
		VIEW_TTL.achievements,
		);
		...
		const cachedGlobalPcts = readViewCache(
		id,
		"globalPcts",
		VIEW_TTL.globalPcts,
		);
