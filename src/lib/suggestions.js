...
export function buildChangeOfPaceSuggestions(games, preferred = [], excluded = [], alreadySuggested = []) {
	const usedAppIds = new Set(alreadySuggested.map(item => appKey(item.game?.steam_appid ?? item.appid)))
	const weights = buildGenreWeights(games)
	const maxWeight = Math.max(...weights.values(), 1)
	const preferredGenres = lowerList(preferred)
	...
	return playableLibraryEntries(games, excluded)
		...
		.map(({ game, playtime }) => {
			const genres = genreNames(game)
			const lowerGenres = lowerList(genres)
			const preferredHit = lowerGenres.some(genre => preferredGenres.includes(genre))
			const familiarHits = lowerGenres.filter(genre => topGenres.has(genre)).length
			const varietyGenre = genres.find(genre => !topGenres.has(genre.toLowerCase())) ?? genres[0] ?? null
			const lowPlaytimeScore = playtime === 0 ? maxWeight * 1.8 : Math.max(0, maxWeight - Math.log((playtime / 60) + 1) * 18)
			const varietyScore = Math.max(0, genres.length - familiarHits) * maxWeight * 0.45
			const preferenceAnchor = preferredHit ? maxWeight * 0.35 : 0
			const qualityScore = (game.metacritic_score ?? 0) * (maxWeight / 120)

			return {
				game,
				score: lowPlaytimeScore + varietyScore + preferenceAnchor + qualityScore,
				reason: varietyGenre
					? `${varietyGenre} change of pace from your usual rotation`
					: 'Change-of-pace pick from your library',
			}
		})
